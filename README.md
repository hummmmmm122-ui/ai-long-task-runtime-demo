...
3. 在右侧 `临时介入` 输入框补充约束，确认用户可以在主任务不暂停的情况下继续参与。
4. 点击右侧 `采纳到当前节点`，确认用户意见可以被吸收进当前任务，而不是只能等待最终结果。
5. 点击 `保留并开分支`，确认当用户希望改动已完成节点时，可以保留当前版本并开启新分支。
6. 查看 `V1 / V2 对比`，确认系统能解释当前版本和新分支的差异。
7. 点击顶部 `继续执行`，任务进入后续节点，节点状态和主面板内容同步变化。
