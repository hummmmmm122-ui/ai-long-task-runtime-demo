...
预览图展示的是默认运行入口：AI 正在执行 `生成方案` 节点，用户可以在右侧分支聊天中选择采纳意见或保留当前版本后开启新分支。
...
2. 点击右侧 `采纳到当前节点`，确认用户意见可以被吸收进当前任务，而不是只能等待最终结果。
3. 点击 `保留并开分支`，确认当用户希望改动已完成节点时，可以保留当前版本并开启新分支。
4. 点击顶部 `继续执行`，任务进入后续节点，节点状态和主面板内容同步变化。
5. 点击 `聚焦视图`，把运行详情和节点产物作为主要工作区查看。
